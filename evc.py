...
def main():
    ...
    try:
        frequency = tk.StringVar(value=f"{1 / abs(TIU_INTERVAL):.3f}")
    except ZeroDivisionError:
        frequency = tk.StringVar(value="1")
    ...
    tk.Label(root, text="Frekvence\nodesílání [ms]").grid(row=2, column=0)
    tk.Entry(root, textvariable=frequency, width=7).grid(row=2, column=1, columnspan=2, sticky="w")
    ...
    def set_interval(_):
        root.focus()
        try:
            frq = float(frequency.get())
        except ValueError as e:
            tkinter.messagebox.showwarning("Chyba", e)
        else:
            client._interval = 1 / frq
